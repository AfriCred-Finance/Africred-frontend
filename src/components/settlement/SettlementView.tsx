...
export type SettlementVariant = "open" | "whitelisted";

export function SettlementView({ variant }: { variant: SettlementVariant }) {
  const addrs = useChainAddresses();
  const { usdc, chainName, explorer } = addrs;
  const settlementVault =
    variant === "whitelisted" ? addrs.whitelistedSettlementVault : addrs.settlementVault;
    ...
    <>
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div>
              <div className="eyebrow !text-[10px]">
                {isWhitelistedVariant ? "Whitelisted settlement vault" : "Settlement vault"}
              </div>
              <h1 className="mt-2 text-3xl font-light tracking-[-0.02em] lg:text-4xl">
                {isWhitelistedVariant ? "Whitelisted trade pool" : "Trade settlement pool"}
              </h1>
              <p className="mt-2 max-w-xl text-sm text-ink2">
                {isWhitelistedVariant
                  ? "Same mechanics as the open vault, but only owner-approved wallets can deposit. Withdraw, request, and claim stay unrestricted."
                  : "LPs deposit USDC; the allocator draws to pre-finance orders and repays with spread. Withdrawals hit the reserve first; anything above queues 24h FIFO."}
              </p>
              <a
      ...
      {(isAllocator || isOwner) && (
        <AdminSection
          ...
          isOwner={isOwner}
          isWhitelistedVariant={isWhitelistedVariant}
          onSuccess={refetchAll}
        />
      )}
    </>
...
type AdminTab = "operations" | "admin" | "whitelist";
  ...
  return (
    <section className="hairline border-b">
      <div className="mx-auto max-w-content px-6 py-10 lg:px-12">
        ...
        {active === "whitelist" && isOwner && isWhitelistedVariant && (
          <WhitelistPanel vault={vault} onSuccess={onSuccess} />
        )}
      </div>
    </section>
  );
...
function WhitelistPanel({
  ...
  const [batchAllow, setBatchAllow] = useState(true);

  const singleAllowA = useAction(() => {
    setSingleAddr("");
    onSuccess();
  });
  const singleBlockA = useAction(() => {
    setSingleAddr("");
    onSuccess();
  });
  const batchA = useAction(() => {
    setBatchText("");
    onSuccess();
  });
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {/* Single approve/block */}
      <div className="card p-5">
        <div className="mb-3 text-xs uppercase tracking-wider text-muted">Approve or block a wallet</div>
        <input
          type="text"
          className="input"
          placeholder="0x…"
          ...
          <button
            ...
            onClick={() =>
              singleAllowA.run({
                ...
                args: [singleAddr as Address, true],
              })
            }
            disabled={singleAllowA.pending || !/^0x[a-fA-F0-9]{40}$/.test(singleAddr)}
          >
            {singleAllowA.pending ? "Approving…" : "Approve"}
          </button>
          <button
            className="btn"
            onClick={() =>
              singleBlockA.run({
                ...
                abi: settlementVaultAbi,
                functionName: "setWhitelist",
                args: [singleAddr as Address, false],
              })
            }
            disabled={singleBlockA.pending || !/^0x[a-fA-F0-9]{40}$/.test(singleAddr)}
          >
            {singleBlockA.pending ? "Blocking…" : "Block"}
          </button>
        </div>
        <ErrorLine error={singleAllowA.error || singleBlockA.error} />
      </div>

      {/* Check status */}
      <div className="card p-5">
        <div className="mb-3 text-xs uppercase tracking-wider text-muted">Check whitelist status</div>
        <input
          type="text"
          className="input"
          placeholder="0x…"
        ...
        <div className="mt-3 text-sm">
          {/^0x[a-fA-F0-9]{40}$/.test(checkAddr) ? (
            checkResult === undefined ? (
              <span className="text-ink3">Loading…</span>
            ) : checkResult ? (
              <span className="tag border-accent/40 text-accent">Whitelisted</span>
            ) : (
              <span className="tag border-rule text-ink3">Not whitelisted</span>
            )
          ) : (
            <span className="text-ink3">Paste a full address.</span>
          )}
        </div>
      </div>
      ...
      <div className="card p-5 md:col-span-2">
        <div className="mb-3 flex items-baseline justify-between">
          <div className="text-xs uppercase tracking-wider text-muted">Batch update</div>
          <div className="font-mono text-[11px] text-ink3">
            {parsedBatch.length} valid address{parsedBatch.length === 1 ? "" : "es"}
          </div>
        </div>
        <textarea
          className="input min-h-[80px] w-full font-mono text-xs"
          placeholder="One or many addresses, separated by comma, space, semicolon, or newline"
          value={batchText}
          onChange={(e) => setBatchText(e.target.value)}
        />
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <label className="inline-flex items-center gap-2 text-sm text-ink2">
            <input
              type="radio"
              checked={batchAllow}
              onChange={() => setBatchAllow(true)}
            />
            Approve all
          </label>
          <label className="inline-flex items-center gap-2 text-sm text-ink2">
          ...
          <button
            ...
            onClick={() =>
              batchA.run({
                ...
                args: [parsedBatch, batchAllow],
              })
            }
            disabled={batchA.pending || parsedBatch.length === 0}
          >
            {batchA.pending ? "Submitting…" : `Apply to ${parsedBatch.length}`}
          </button>
        </div>
        <ErrorLine error={batchA.error} />
      </div>
    </div>
  );
}
